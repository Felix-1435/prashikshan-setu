...
export default function Quizzes({ user }: { user: User }) {
  ...
  return (
    <div className="space-y-6 rise">
      <div>
        <h1 className="font-display text-3xl heading">{isStaff ? "Quiz bank" : "My assessments"}</h1>
        <p className="text-sm muted mt-1">
          {isStaff
            ? "Review generated quizzes and answer keys. Trainee scores appear under Scores & overview."
            : "Complete assigned quizzes. Finished items show your latest score."}
        </p>
      </div>
      <div className="space-y-3">
              ...
              <Link href={`/quizzes/${r.id}`}>
                <a className="card card-lift p-4 flex items-center gap-3 block">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0"
                    ...
                    {isStaff ? <Eye className="w-5 h-5" style={{ color: "var(--accent)" }} /> : <ClipboardList className="w-5 h-5" style={{ color: "var(--accent)" }} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold truncate heading">{r.title}</div>
                    <div className="text-xs muted">{r.question_count} questions · {r.domain || "Training"} · {isStaff ? "Review mode" : "Trainee attempt"}</div>
                  </div>
                  {att && (
                    <span className="pill" style={{ background: "color-mix(in srgb, var(--ok) 16%, transparent)", color: "var(--ok)" }}>
        ...
        {rows.length === 0 && <p className="text-sm muted">No quizzes yet. Coordinators create them under Build quiz.</p>}
      </div>
    </div>
  );
}
